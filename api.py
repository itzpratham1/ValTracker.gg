"""
ValTracker API — Flask API-only backend for the Astro + Svelte frontend.
Serves /api/* routes only. Frontend is served by Astro (Node.js).
"""
import os
import time
import requests
...
if not ADMIN_SECRET:
    ...
    print("[SECURITY WARNING] ADMIN_SECRET not set in environment! A random secret has been generated for this session.")
...
# ── FLASK APP (API-only, no static file serving) ──
...
def optimize_response(response):
    ...
    compressible = ["text/html", "text/css", "application/javascript", "application/json", "text/javascript"]
    is_compressible = any(mime in content_type for mime in compressible)
    if not is_compressible:
        return response
    response.direct_passthrough = False
    data = response.get_data()
    if len(data) < 500 or len(data) > 2 * 1024 * 1024:
        return response
...
@app.route("/")
def root_health():
    return jsonify({"status": "ok", "service": "valtracker-api", "endpoint": "root"})
...
def share_card_api():
    try:
        ...
        # Save to shared directory (relative to project root)
        ...
        for m in live_matches_data["data"]:
            ...
            if me:
                ...
                if not actual_m_mode:
                    actual_m_mode = mode
                match_payload = {
                    "puuid": puuid, "match_id": match_id, "mode": actual_m_mode,
                    "map_name": map_name, "game_start": game_start, "agent_name": agent_name,
                    "kills": kills, "deaths": deaths, "assists": assists, "score": score,
                    "won": won, "team": team, "rounds_summary": rounds_str,
                        ...
                        try:
                            ...
                            c_match = {
                                "metadata": {
                                    ...
                                    "mode": meta.get("mode"), "queue": meta.get("mode"),
                                    "season": meta.get("season", {}).get("short"),
                                    "rounds_played": my_score + opp_score
                                },
                                "players": {
                                    "all_players": [{
                                        "puuid": stats.get("puuid") or puuid, "name": name, "tag": tag,
                                        "team": my_team, "character": char_name,
                                        "currenttier": stats.get("tier", 0), "currenttier_patched": "",
                                        "stats": {"score": score, "kills": kills, "deaths": deaths, "assists": assists,
                                ...
                                "teams": {
                                    "red": {"has_won": t_scores.get("red", 0) > t_scores.get("blue", 0), "rounds_won": t_scores.get("red", 0)},
                                    "blue": {"has_won": t_scores.get("blue", 0) > t_scores.get("red", 0), "rounds_won": t_scores.get("blue", 0)}
                                },
                                "rounds": []
                            }
            ...
            for item in match_items:
                ...
                for t_div in teams_divs:
                    ...
                    try:
                        ...
                        score = 0
                    teams_data.append({"name": name, "game_wins": score, "has_won": False})
                if len(teams_data) < 2:
                ...
                matches.append({
                    ...
                    "state": state,
                    "vlr_path": href,
                    "league": {"name": event_name, "region": stage_name},
                    "match": {"id": match_id, "teams": teams_data},
                    "vod": None
                })
        try:
...
def esports_news():
    ...
    mock_news = [
        {"title": "VCT Masters London: Qualifiers and Contenders locked in", "description": "With Stage 1 concluding across all regions, the 12 teams representing Pacific, Americas, EMEA, and China are set for the London LAN.", "date": "1 day ago", "author": "ValTracker News", "url_path": ""},
...
def api_meta_comps():
    ...
    for agent, count in agent_appearances.items():
        ...
        agent_stats[agent] = {"pick_rate": pick_rate, "win_rate": win_rate, "raw_picks": count, "raw_wins": agent_wins.get(agent, 0)}
    comp_stats = {}
    for r in patch_records:
