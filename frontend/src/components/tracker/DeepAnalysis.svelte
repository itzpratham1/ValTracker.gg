...
<style>
  .deep-wrap { margin-top: 16px; }

  .deep-clarification-banner {
    background: linear-gradient(135deg, rgba(56, 189, 248, 0.08) 0%, rgba(20, 20, 22, 0.6) 100%);
  ...
  .deep-clar-icon { font-size: 22px; filter: drop-shadow(0 0 6px #0ea5e9); }

  .deep-clar-title {
    font-family: 'Barlow Condensed', sans-serif; font-weight: 800; font-size: 13px;
    text-transform: uppercase; color: #38bdf8; letter-spacing: 0.5px; margin-bottom: 2px;
  }

  .deep-clar-desc { font-size: 11px; color: var(--muted); line-height: 1.4; }
  .deep-clar-desc :global(strong) { color: var(--text); }
  ...
  .deep-run-btn {
    ...
    background: #38bdf8; color: #0d0d0f; border: none; border-radius: var(--radius-sm);
    cursor: pointer; transition: var(--transition);
  }

  .deep-run-btn:hover { background: #7dd3fc; transform: translateY(-2px); }
  .deep-run-btn:active { transform: translateY(0) scale(0.97); }
  .deep-run-btn:disabled { opacity: 0.4; pointer-events: none; }

  .deep-loading {
    display: none; align-items: center; gap: 10px; padding: 14px 0;
    font-family: 'DM Mono', monospace; font-size: 10px; color: var(--muted);
    letter-spacing: 2px;
  }

  .deep-loading :global(.active) { display: flex; }

  .deep-spinner {
    width: 16px; height: 16px; border-radius: 50%;
    border: 2px solid var(--border2); border-top-color: #38bdf8;
  ...
  .deep-results.active { display: block; }

  .deep-results :global(.deep-chapter) {
    display: flex; align-items: center; gap: 10px; margin: 24px 0 12px;
  }

  .deep-results :global(.deep-chapter-icon) { font-size: 18px; }

  .deep-results :global(.deep-chapter-title) {
  ...
  .deep-results :global(.deep-chapter-line) {
    flex: 1; height: 1px; background: var(--border);
  }

  .deep-results :global(.deep-card) {
    background: var(--surface2); border: 1px solid var(--border); border-radius: var(--radius-sm);
    padding: 14px; transition: border-color 0.2s;
  }

  .deep-results :global(.deep-card:hover) { border-color: rgba(255,255,255,0.12); }
  .deep-results :global(.deep-card.accent-left) { border-left: 3px solid var(--accent); }
  .deep-results :global(.deep-card.accent-green) { border-left: 3px solid var(--win); }
  .deep-results :global(.deep-card.accent-red) { border-left: 3px solid var(--loss); }
  .deep-results :global(.deep-card.accent-blue) { border-left: 3px solid var(--blue); }
  .deep-results :global(.deep-card.span3) { grid-column: span 3; }

  .deep-results :global(.deep-card-label) {
    font-family: 'DM Mono', monospace; font-size: 9px; color: var(--muted);
    letter-spacing: 2px; text-transform: uppercase; margin-bottom: 6px;
  }

  .deep-results :global(.deep-card-val) {
    font-family: 'Barlow Condensed', sans-serif; font-weight: 900; font-size: 36px;
    line-height: 1; letter-spacing: -0.5px;
  }

  .deep-results :global(.deep-card-val.good) { color: var(--win); }
  .deep-results :global(.deep-card-val.warn) { color: #f5a623; }
  .deep-results :global(.deep-card-val.bad) { color: var(--loss); }
  .deep-results :global(.deep-card-val.accent) { color: var(--accent); }

  .deep-results :global(.deep-card-sub) {
    font-family: 'DM Mono', monospace; font-size: 10px; color: var(--muted);
    margin-top: 4px; letter-spacing: 0.5px;
  }

  .deep-results :global(.deep-insight-grid) {
    display: grid; gap: 10px; margin-bottom: 12px;
  }

  .deep-results :global(.deep-insight-grid.cols2) { grid-template-columns: repeat(2, 1fr); }
  .deep-results :global(.deep-insight-grid.cols3) { grid-template-columns: repeat(3, 1fr); }
  .deep-results :global(.deep-insight-grid.cols4) { grid-template-columns: repeat(4, 1fr); }

  .deep-results :global(.deep-pattern-list) { display: flex; flex-direction: column; gap: 8px; }

  .deep-results :global(.deep-pattern-item) {
    display: flex; align-items: flex-start; gap: 8px;
    font-family: 'Barlow', sans-serif; font-size: 12px; line-height: 1.5;
    color: rgba(240, 240, 242, 0.85);
  }

  .deep-results :global(.deep-pattern-dot) {
    width: 6px; height: 6px; border-radius: 50%; flex-shrink: 0; margin-top: 5px;
  }

  .deep-results :global(.deep-pattern-dot.g) { background: var(--win); }
  .deep-results :global(.deep-pattern-dot.r) { background: var(--loss); }
  .deep-results :global(.deep-pattern-dot.y) { background: #f5a623; }

  .deep-results :global(.deep-map-table) {
    width: 100%; border-collapse: collapse; margin-top: 8px; font-size: 12px;
  }

  .deep-results :global(.deep-map-table th) {
    font-family: 'DM Mono', monospace; font-size: 9px; color: var(--muted);
    letter-spacing: 1px; text-transform: uppercase; text-align: left;
    padding: 6px 8px; border-bottom: 1px solid var(--border);
  }

  .deep-results :global(.deep-map-table td) {
    padding: 8px; border-bottom: 1px solid rgba(255,255,255,0.03);
    font-family: 'Barlow Condensed', sans-serif; font-size: 13px;
  }

  .deep-results :global(.deep-map-row-name) {
    font-weight: 700; margin-right: 6px;
  }

  .deep-results :global(.deep-map-verdict) {
    font-family: 'DM Mono', monospace; font-size: 8px; padding: 1px 4px;
    border-radius: 3px; letter-spacing: 0.5px;
  }

  .deep-results :global(.deep-map-verdict.strong) { background: rgba(16,185,129,0.15); color: var(--win); }
  .deep-results :global(.deep-map-verdict.avg) { background: rgba(245,166,35,0.12); color: #f5a623; }
  .deep-results :global(.deep-map-verdict.weak) { background: rgba(244,63,94,0.12); color: var(--loss); }

  .deep-results :global(.deep-mismatch-row) {
    display: flex; align-items: center; gap: 8px; padding: 8px 0;
    border-bottom: 1px solid rgba(255,255,255,0.03); flex-wrap: wrap;
    font-family: 'Barlow Condensed', sans-serif; font-size: 13px;
  }

  .deep-results :global(.deep-mismatch-agent) { font-weight: 700; color: var(--text); }
  .deep-results :global(.deep-mismatch-on) { color: var(--muted); font-size: 11px; }
  .deep-results :global(.deep-mismatch-stat) { font-family: 'DM Mono', monospace; font-size: 10px; }
  .deep-results :global(.deep-mismatch-tag) {
    font-family: 'DM Mono', monospace; font-size: 8px; padding: 1px 4px;
    border-radius: 3px; letter-spacing: 0.5px;
  }
  .deep-results :global(.deep-mismatch-tag.bad) { background: rgba(244,63,94,0.12); color: var(--loss); }
  .deep-results :global(.deep-mismatch-tag.ok) { background: rgba(16,185,129,0.12); color: var(--win); }
  .deep-results :global(.deep-mismatch-tip) { font-size: 11px; color: var(--muted2); width: 100%; }

  .deep-results :global(.deep-priority) {
    background: var(--surface2); border: 1px solid var(--border); border-radius: var(--radius-sm);
    padding: 16px; margin-top: 12px;
  }

  .deep-results :global(.deep-priority-label) {
    font-family: 'DM Mono', monospace; font-size: 10px; color: var(--accent);
    letter-spacing: 2px; text-transform: uppercase; margin-bottom: 12px;
  }

  .deep-results :global(.deep-priority-list) { display: flex; flex-direction: column; gap: 12px; }

  .deep-results :global(.deep-priority-item) {
    display: flex; gap: 12px; align-items: flex-start;
  }

  .deep-results :global(.deep-priority-num) {
  ...
  .deep-results :global(.deep-priority-title) {
    font-family: 'Barlow Condensed', sans-serif; font-weight: 700; font-size: 14px;
    color: var(--text); letter-spacing: 0.5px;
  }

  .deep-results :global(.deep-priority-desc) {
    font-family: 'Barlow', sans-serif; font-size: 12px; color: var(--muted);
    line-height: 1.5; margin-top: 2px;
  }

  @media (max-width: 900px) {
    .deep-results :global(.deep-insight-grid.cols4) { grid-template-columns: repeat(2, 1fr); }
    .deep-results :global(.deep-card.span3) { grid-column: span 2; }
  }

  @media (max-width: 600px) {
    .deep-results :global(.deep-insight-grid.cols4),
    .deep-results :global(.deep-insight-grid.cols3) { grid-template-columns: 1fr; }
    .deep-results :global(.deep-insight-grid.cols2) { grid-template-columns: 1fr; }
    .deep-results :global(.deep-card.span3) { grid-column: span 1; }
  }
</style>
