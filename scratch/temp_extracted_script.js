...
async function fetchAll(){
  ...
  try{
    ...
    const[mmrRes,matchRes,accountRes,mmrHistRes]=await Promise.all([
      ...
      fetch(`/api/v3/matches/${region}/${enc}/${PLAYER_TAG}?mode=${mode}&size=100&_nocache=${_nc}`),
...
function buildMatchAnalysis(match, allMatches = null) {
  ...
  return `
    ...
    <!-- Match Intel AI Clarification Banner -->
...
function _saveWeaponSnapshot(weaponData) {
  if (!PLAYER_NAME || !PLAYER_TAG) return;
  const key = `vt_wpn_hist_${PLAYER_NAME.toLowerCase()}_${PLAYER_TAG.toLowerCase()}`;
  let history = [];
...
function renderTopWeapons(matches) {
  ...
  if (topWeapon) {
    ...
    const sparkSVG = _buildSparklineSVG(trendInfo.sparkPoints, 120, 32);
    const sessionCount = trendInfo.sparkPoints.length;
    ...
    const sparklineBlock = sparkSVG
      ? `<div class="wpn-sparkline-section">
           <div class="wpn-sparkline-header">
             <span class="wpn-sparkline-label">HS% TREND</span>
             <span class="wpn-sparkline-sessions">${sessionCount} session${sessionCount !== 1 ? 's' : ''} tracked</span>
           </div>
           <div class="wpn-sparkline-wrap">${sparkSVG}</div>
           <div class="wpn-sparkline-axis">
             <span>${Math.min(...trendInfo.sparkPoints)}%</span>
             <span class="wpn-sparkline-axis-mid">across ${sessionCount} fetches</span>
             <span>${Math.max(...trendInfo.sparkPoints)}%</span>
           </div>
         </div>`
      : `<div class="wpn-sparkline-section wpn-first-session">
           <span class="wpn-sparkline-label">&#128202; TRACKING STARTED</span>
           <span class="wpn-sparkline-sessions">Trend data builds across your next sessions</span>
         </div>`;
    ...
    showcaseHtml += `
      <div class="top-weapon-showcase" style="display: flex; flex-direction: column; justify-content: space-between;">
        ...
        <div style="display: flex; gap: 16px; align-items: stretch; justify-content: space-between; flex: 1;">
  ...
  secondaryWeapons.forEach(([wpn, v]) => {
    ...
    showcaseHtml += `
      <div class="secondary-weapon-row">
        ...
        <div class="sec-weapon-acc-bars wpn-tooltip-trigger">
          ...
          <div class="wpn-tooltip-card">
            <div style="font-family:'Barlow Condensed', sans-serif; font-size:10px; font-weight:800; text-transform:uppercase; color:var(--accent); margin-bottom:4px; letter-spacing:0.5px;">Target Heatmap</div>
            ${getHeatmapSvg(hsPct, 64)}
            <div style="display:flex; justify-content:space-between; width:100%; font-family:'DM Mono',monospace; font-size:8px; color:#fff; border-top:1px solid rgba(255,255,255,0.06); padding-top:4px; margin-top:3px; box-sizing:border-box;">
  ...
  for (const region in vctFranchiseData) {
    for (const t of vctFranchiseData[region]) {
      if (t.name.toLowerCase() === name.toLowerCase() || t.tag?.toLowerCase() === name.toLowerCase()) {
        ...
        return `<img src="${teamLogoUrl}" style="width:20px;height:20px;object-fit:contain;margin-right:8px;" onerror="this.outerHTML='${fallbackEscaped}';" />`;
      }
    }
  }
